...
def conv7x7(in_planes, out_planes, padding=3, stride=1, groups=1, dilation=1, experts=5, align=False, lite=False, replace_pro=0.2):
    return AdaAggLayer(
        in_planes, out_planes, kernel_size=7, stride=stride, 
        padding=padding, groups=groups, bias=False, dilation=dilation,
        experts=experts, align=align, lite=lite, replace_pro=replace_pro
    )


def conv3x3(in_planes, out_planes, stride=1, groups=1, dilation=1, experts=5, align=False, lite=False, replace_pro=0.2):
    return AdaAggLayer(
...
def conv1x1(in_planes, out_planes, stride=1, experts=5, align=False, lite=False, replace_pro=0.2):
    return AdaAggLayer(
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None, experts=5, align=False, lite=False, replace_pro=0.2):
        super(Bottleneck, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        width = int(planes * (base_width / 64.)) * groups
        # Both self.conv2 and self.downsample layers downsample the input when stride != 1
        self.conv1 = conv1x1(inplanes, width, experts=experts, align=align, lite=lite, replace_pro=replace_pro)
        self.bn1 = norm_layer(width)
        self.conv2 = conv3x3(width, width, stride, groups, dilation, experts=experts, align=align, lite=lite, replace_pro=replace_pro)
        self.bn2 = norm_layer(width)
        self.conv3 = conv1x1(width, planes * self.expansion, experts=experts, align=align, lite=lite, replace_pro=replace_pro)
    ...
    def forward(self, xs):
        ...
        out = self.conv1((x, expert_i))
        out = self.bn1(out)
        out = self.relu(out)

        out = self.conv2((out, expert_i))
        out = self.bn2(out)
        out = self.relu(out)

        out = self.conv3((out, expert_i))
        out = self.bn3(out)

        if self.downsample is not None:
            identity = self.downsample((x, expert_i))
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 ...
                 norm_layer=None, experts=5, align=True, lite=False, replace_pro=0.2):
        ...
        self.conv1 = conv7x7(3, self.inplanes, stride=2, padding=3, experts=experts, align=align, lite=lite, replace_pro=replace_pro)
        self.bn1 = norm_layer(self.inplanes)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(block, 64, layers[0], 
                                       experts=experts, lite=lite, replace_pro=replace_pro)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False, experts=5, align=False, lite=False, replace_pro=0.2):
    ...
    def forward(self, x, expert_i=0):
        x = self.conv1((x, expert_i))
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)

        x = self.layer1((x, expert_i))
        x = self.layer2((x[0], expert_i))
        x = self.layer3((x[0], expert_i))
        x = self.layer4((x[0], expert_i))
