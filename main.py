...
def get_configs():
    ...
    parser.add_argument('--replace_pro', default=0.2, type=float)
...
def get_data_loader(configs):
    ...
    if configs.dataset == 'cifar':
        train_dataset = datasets.CIFAR100(root='./data3', train=True, transform=data_transforms['train'], download=True)
        val_dataset = datasets.CIFAR100(root='./data3', train=False, transform=data_transforms['val'])
        test_datasets = {
            'test' + str(i):
                datasets.CIFAR100(root='./data3', train=False, transform=data_transforms['test' + str(i)])
            for i in range(10)
        }
        class_num = 100
...
def train(configs, train_loader, val_loader, test_loaders, net):
    ...
    for iter_num in range(configs.total_iter):
        ...
        train_outputs = [net(train_inputs, expert_i=i) for i in range(6)]
        
        train_acc_meter.update(train_outputs[0], train_labels)

        classifier_loss = nn.CrossEntropyLoss()(train_outputs[0], train_labels)
        loss = sum([nn.CrossEntropyLoss()(train_outputs[i], train_labels) for i in range(6)]) / 6
        ...
        if iter_num % configs.eval_iter == 0 and iter_num > 0:
            ...
            with torch.no_grad():
                ...
                for val_inputs, val_labels in tqdm(val_loader):
                    ...
                    val_outputs = net(val_inputs, expert_i=0)
...
def main():
    ...
    class Net(nn.Module):
        def __init__(self):
            ...
            self.f_net = build_model(lite=configs.lite, replace_pro=configs.replace_pro)
            self.c_net = nn.ModuleList([nn.Linear(2048, class_num) for _ in range(6)])
            for i in range(5):
                self.c_net[i].weight.data.normal_(0, 0.01)
                self.c_net[i].bias.data.fill_(0.0)
                
        def forward(self, x, expert_i):
            feature = self.f_net(x, expert_i)
            out = self.c_net[expert_i](feature)
